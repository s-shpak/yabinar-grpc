...
import (
	...
	pb "webinar-service/internal/protos/v3/dummy"
	pbModel "webinar-service/internal/protos/v3/dummy/model"
...
func transformMessage(msg *pbModel.HelloMessage, tfs []*pbModel.HelloTransformation) string {
	...
	for _, tf := range tfs {
		switch tf.Type {
		case pbModel.HelloTransformationType_HELLO_TRANSFOMRATION_TYPE_TO_UPPER.Enum():
			innerMsg = proto.String(strings.ToUpper(*innerMsg))
		case pbModel.HelloTransformationType_HELLO_TRANSFOMRATION_TYPE_REVERSE.Enum():
			innerMsg = proto.String(reverse(*innerMsg))
		}
	}
